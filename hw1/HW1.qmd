...
completion:
https://www.citiprogram.org/verify/?k726a5b1e-99a1-4c08-a9ae-f06b5368b777-53983534

certificate:
https://www.citiprogram.org/verify/?wa12cf624-d5cd-46a4-a58b-53f18b26a146-53983534
...
ls -l 
```
Since when we compress file, we could also reduce the file size. 

3. Briefly describe what bash commands `zcat`, `zless`, `zmore`, and `zgrep` do.

These five commands are to display a compressed file and to allowing the user to expand and view a compressed file without uncompressing that file.

4. What's the output of the following bash script?

show all the file name include .gz
```{bash}
#| eval: false
cd ..
cd ..
...
cd mimic-iv-1.0
cd core
more admissions.csv.gz | iconv -f GBK -t UTF-8
head admissions.csv.gz
wc -l admissions.csv.gz
awk '{print $1}' admissions.csv.gz | sort | uniq | wc -l
```
...
cd core
more admissions.csv.gz | iconv -f GBK -t UTF-8
awk '{print $6}' admissions.csv.gz | sort | uniq | wc -l
awk '{print $7}' admissions.csv.gz | sort | uniq | wc -l
awk '{print $9}' admissions.csv.gz | sort | uniq | wc -l
...
Wget is a computer tool to retrieve content and files from various web servers
nc command is a command-line utility for reading and writing data between two computer networks
```{bash}
#| eval: false
...
bash middle.sh pg42671.txt 20 5
```
Explain the output. Explain the meaning of `"$1"`, `"$2"`, and `"$3"` in this shell script. Why do we need the first line of the shell script?

To identify the shell to use
...
#show 2021 calender#
cal 2021
#show 1752 septembers calender
cal 9 1752
#today#
date

#host name of server who currently log in
hostname
#hardware name of linux
arch
#info of system
uname -a
#running time
...
#status
who
#process by log-in user
w
#id 
id
#last ten logged-in users
last | head
#display line of text/string that are passed as an argument .
echo {con,pre}{sent,fer}{s,ed} 
#check the shell has been stopped for five sec.
time sleep 5 
# last ten commands
history | tail
