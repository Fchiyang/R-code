...
chartevents_tble <- chartevents_tble %>%
  ...
  left_join(select(d_labitems_tble, itemid), by = c("itemid")) %>%
  print(width = Inf)
```
...
chartevents_tble <- chartevents_tble %>%
  ...
  pivot_wider(names_from = itemid, values_from = valuenum, names_prefix = "chart") %>%
  print(width = Inf)
```
...
mimic_icu_cohort <- icustays_tble %>%
    ...
    mutate(thirty_day_mort = 
             ifelse(is.na(dod), "FALSE", 
                    ifelse(as.Date(dod) - as.Date(admittime) <= 30, 
                           "TRUE", "FALSE"))) %>%
    print(width = Inf) %>%
